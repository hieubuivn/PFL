...
export class Helper {
    ...
    _createPanel() {
        ...
        Object.assign(this.Status.style, {
            ...
            padding: '0', borderRadius: '5px', fontFamily: 'monospace', fontSize: '11px', zIndex: '999999',
    ...
    _createSceneSection() {
        ...
        // Environment Rotation (Sliders -4 to 4 Rad)
        this._envRotSliders = {};
        ['x', 'y', 'z'].forEach(axis => {
            const label = `Env Rot ${axis.toUpperCase()}`;
            const slider = this._createUI_Slider(label,
                () => this.scene.environmentRotation ? this.scene.environmentRotation[axis] : 0,
                (val) => {
                    if (!this.scene.environmentRotation) this.scene.environmentRotation = new THREE.Euler();
                    this.scene.environmentRotation[axis] = val;
                },
                { min: -4, max: 4, step: 0.01 }
            );
            this._envRotSliders[axis] = slider;
            this.sceneSection.appendChild(slider.row);
        });
    }
    ...
    _createRendererSection() {
        ...
        this.exposureControls = this._createUI_Slider('ToneMapping Exposure:', () => this.renderer.toneMappingExposure, v => this.renderer.toneMappingExposure = v, { min: 0, max: 20, step: 0.1 });
    ...
    _createResetButton() {
        ...
        this.resetBtn.onclick = () => {
            if (confirm('Reset all settings to default?')) {
                this._resets.forEach(reset => reset());
            }
        };
    }
    ...
    _createUI_Row(name, obj, isResizable, creator) {
        ...
        if (isResizable) {
            ...
            slider.type = 'range'; slider.min = 1; slider.max = Math.max(10, Math.round(initialVal * 10)); slider.value = initialVal;
    ...
    _createUI_Slider(labelText, getValue, setValue, { min, max, step }) {
        ...
        const initialVal = getValue() ?? 1;
        const slider = document.createElement('input'); slider.type = 'range'; slider.min = min; slider.max = max; slider.step = step; slider.value = initialVal;
        ...
        slider.addEventListener('wheel', (e) => {
            e.preventDefault();
            const delta = Math.sign(e.deltaY) * -1;
            const newVal = Math.min(max, Math.max(min, parseFloat(slider.value) + (delta * step)));
            slider.value = newVal;
            update();
        });
    ...
    _createUI_Select(labelText, getValue, setValue, options) {
        ...
        options.forEach(opt => {
            ...
            if (initialVal === opt.value) option.selected = true;
        ...
        this._resets.push(() => {
            ...
            setValue(select.value); // Trigger update
        });
    ...
    _createUI_ColorPicker(labelText, getValue, setValue) {
        ...
        const initialHex = '#' + (getValue() ? getValue().getHexString() : '000000');
    ...
    _setupInteractivity() {
        ...
        const onMouseUp = () => {
            if (isDragging) {
                ...
                // Small delay to prevent click from firing immediately if it was a drag
        ...
        this.head.onmousedown = (e) => {
            ...
            // Switch to fixed positioning at the current location to allow smooth dragging
        ...
        this.head.onclick = (e) => {
            ...
            if (!collapsed) {
                this.update();
                // Restore to bottom-left when expanded, as requested
                this.Status.style.position = 'fixed';
                this.Status.style.left = '10px';
                this.Status.style.bottom = '10px';
                this.Status.style.top = '';
                this.Status.style.right = '';
            }
        };
    }
    ...
    update() {
        ...
        if (this._envRotSliders && this.scene.environmentRotation) {
            const rot = this.scene.environmentRotation;
            ['x', 'y', 'z'].forEach(axis => {
                const slider = this._envRotSliders[axis];
                if (slider) {
                    const val = rot[axis];
                    slider.slider.value = val;
                    slider.valueDisplay.textContent = val.toFixed(2);
                }
            });
        }
