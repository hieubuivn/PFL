...
`,an=Mt(`#FBC189`,1,1),an.name=`goldInner`,an.clone(),on=jt(`#FBC189`,1,.01,6.5,e.FrontSide),on.name=`goldOuter`,sn=`
// Simplex 3D Noise 
...
`,ln=Ft(`#FBC189`,1,1,1),Lt(`#FBC189`,1,1,1),un=It(`#FBC189`,1,.01,6.5,e.FrontSide,1),Rt(`#FBC189`,1,.01,6.5,e.FrontSide,1),dn=Bt(`#FBC189`,1,1,.1),fn=`
    uniform vec2 iResolution;
    uniform float iTime;
    uniform vec2 uMouse;
    uniform vec2 uSmoothedMouse;
...
}`}}var Tr,Er,Dr,Or,kr=O((()=>{At(),Tr={core:{iTime:0,iResolution:new e.Vector2(1024,1024)},water:Tt,grid:{...Et,uObjectStagger:0},dotaLogo:Dt,welcome:Ot},Er=(e,t,n,r=`vec2`)=>{let i=`varying ${r} ${n};`;e.vertexShader&&!e.vertexShader.includes(n+`;`)&&(e.vertexShader=e.vertexShader.replace(/void\s+main\s*\(\s*\)\s*\{/,e=>`${i}\n${e}`)),e.fragmentShader&&!e.fragmentShader.includes(n+`;`)&&(e.fragmentShader.includes(`#include <common>`)?e.fragmentShader=e.fragmentShader.replace(`#include <common>`,`#include <common>\n${i}`):e.fragmentShader=`${i}\n${e.fragmentShader}`)},Dr=(e,t,n)=>{let r=t===`vertex`?`vertexShader`:`fragmentShader`;e[r]&&!e[r].includes(n.trim())&&(e[r]=e[r].replace(/void\s+main\s*\(\s*\)\s*\{/,e=>`${e}\n    ${n}`))},Or=(e,t,n)=>{let r=Tr[n];if(r){n!==`core`&&Or(e,t,`core`);for(let[n,i]of Object.entries(r))e.uniforms[n]||(e.uniforms[n]=t[n]||{value:i})}}})),Ar,jr=O((()=>{Ar=`
    uniform vec2 iResolution;
    uniform float iTime;
    uniform float uBSODState; // 0.0 = Normal, 1.0 = BSOD
...
const vec3 BORDER_COLOR = ${As($.BORDER_COLOR)};
uniform vec3 uOutsideColor;
const vec3 ERR_RED = ${js.COLORS.BAD};
    ...
    if (p.x < resX * 0.1) {
        for (int i = 0; i < 2; i++) {
            ...
            if (dBar < 2.0) {
                ...
                float tGood = ${(js.FPS.GOOD/60).toFixed(4)};
                float tNorm = ${(js.FPS.NORMAL/60).toFixed(4)};
...
`}var $,ks,As,js,Ms=O((()=>{hn(),St(),be(),$a(),Es(),$={MARGIN_PCT:.025,VERTICAL_MARGIN_PCT:.5,CUT_SIZE:0,CORNER_RADIUS:4,TL_GAP:5,B_NOTCH_TO_MAIN_W_RATIO:0,B_NOTCH_TO_MAIN_H_RATIO:0,NOTCH_ANGLE:60,R_NOTCH_TO_MAIN_H_RATIO:0,R_NOTCH_TO_MAIN_W_RATIO:0,RIGHT_NOTCH_ANGLE:45,ISL_BAR_COUNT:2,ISL_BAR_HEIGHT_RATIO:.005,ISL_BAR_GAP_RATIO:.01,ISL_BAR_MARGIN_LEFT_RATIO:.1,ISL_BAR_MARGIN_RIGHT_RATIO:.4,ISL_BAR_MARGIN_Y_RATIO:.01,ISL_BAR_PROGRESS:[.15,.42],ISL_TO_MAIN_W_RATIO:-1,NAV_COUNT:6,NAVIGATOR_VISIBILITY:1,NAV_BUTTON_WH_RATIO:2,NAV_CV_BUTTON_WH_RATIO:1,NAV_GAP:5,NAV_FILL_OPACITY:.15,DUR_FILL:1,DUR_HOLD:.5,DUR_WIPE:.25,DUR_PAUSE:.25,DIAMOND_ROT_SPEED:5,BORDER_COLOR:I.ELECTRIC_CYAN,OUTSIDE_COLOR:new e.Color(0,0,0),BORDER_THICK_RATIO:.075,PATTERN_WIDTH:10,PATTERN_GAP:4,PATTERN_LINE_THICK:1,PATTERN_HEIGHT_PCT:.75,BG_GRID_SIZE:60,GRID_LINE_THICKNESS:.6,GRID_PULSE_SPEED:1,GRID_PULSE_DENSITY:1,R_NOTCH_BAR_THICKNESS:2e-4,R_NOTCH_BAR_DIAMOND_SIZE:.00125,R_NOTCH_BAR_PROGRESS:.75,R_NOTCH_BAR_ACTIVE_COLOR:I.ELECTRIC_CYAN,R_NOTCH_BAR_INACTIVE_COLOR:I.ACCENT_GOLD,R_NOTCH_H_RATIO:.4,R_NOTCH_W_RATIO:.02,B_NOTCH_BAR_PROGRESS:1,B_NOTCH_BAR_ALPHA:0,B_NOTCH_BAR_MARGIN_X:.2,B_NOTCH_BAR_MARGIN_Y:.45,B_NOTCH_BAR_COLOR:I.ELECTRIC_CYAN,ELEC_SPEED:256,ELEC_FREQUENCY:4,ELEC_INTENSITY:2,HEAD_SPRITE_SIZE:0,SPRITE_INDEX:26,SPRITE_COLS:8,SPRITE_ROWS:4,HEAD_SCALE:1,SURGE_GRID_SIZE_PX:60,SURGE_GRID_THICK_PX:.4,RING_SPEED:4,RING_INTENSITY:.01,FLOWER_GLOW_HOVER:1.5,GARDEN_HOVER_GRAVITY_MULT:-1,GARDEN_HOVER_TWEEN_DUR:2500,FLOWER_GLOW_BASE:1,FLOWER_COLOR:I.ELECTRIC_CYAN,FLOWER_ROTATION:Math.PI/2,FLOWER_SCALE:.1,GARDEN_IS_FLOWER:0,GROK_SCALE_FACTOR:0,GROK_OFFSET_Y:0,BEAM_SPEED:30,BEAM_FREQ:.512,BEAM_MAX_HEIGHT:.03,BEAM_WAVE_THICKNESS:.0012,BEAM_BASE_THICKNESS:.001,BEAM_TRIM_RATIO:.99,BEAM_GROWTH:1,BEAM_ATTACH_RATIO:1,BEAM_BLOOM:1.5,BEAM_WOBBLE:0,BEAM_GLOW_STRENGTH:.05,BEAM_COLOR:I.ELECTRIC_CYAN,BREATH_AUTO_STRENGTH:.15,BREATH_MANUAL_STRENGTH:.2},ks={hideDeco:{FLOWER_ROTATION:Math.PI/2,HEAD_SPRITE_SIZE:0},showDeco:{FLOWER_ROTATION:0}},As=e=>`vec3(${e.r.toFixed(4)}, ${e.g.toFixed(4)}, ${e.b.toFixed(4)})`,js={FPS:{PERFECT:60,GOOD:54,NORMAL:24,BAD:0},COLORS:{BAD:As(I.CRIMSON_RED)}}}));function Ns(e){let t=`1.5px`,n=`40px`,r=`80px`,i=document.createElement(`div`);Object.assign(i.style,{position:`fixed`,top:`0`,left:`0`,width:`100%`,height:`100%`,pointerEvents:`none`,zIndex:`2000`,overflow:`hidden`});let a=`${($&&$.MARGIN_PCT?$.MARGIN_PCT:.025)*100}vh`,o={position:`absolute`,zIndex:`11`,pointerEvents:`auto`,backgroundColor:`transparent`},s={position:`absolute`,zIndex:`12`,pointerEvents:`auto`,backgroundColor:`rgba(8, 12, 16, 0.9)`,color:`#00F3FF`,border:`1px solid rgba(0, 243, 255, 0.4)`,display:`flex`,justifyContent:`center`,alignItems:`center`,width:r,height:r,boxSizing:`border-box`,backdropFilter:`blur(12px)`,cursor:`pointer`,opacity:`0`,transform:`scale(0.8)`,transition:`all 0.4s cubic-bezier(0.165, 0.84, 0.44, 1)`,boxShadow:`0 0 25px rgba(0, 243, 255, 0.1)`},c=e=>{let n=`width: 6px; height: 6px; background: #00F3FF; transform: rotate(45deg); display: block; box-shadow: 0 0 10px rgba(0, 243, 255, 0.8); transition: all 0.4s cubic-bezier(0.165, 0.84, 0.44, 1);`,r=`display: flex; gap: ${t}; justify-content: center; align-items: center;`,i=`display: flex; flex-direction: column; gap: ${t}; align-items: center; justify-content: center; width: max-content; overflow: visible;`;return e===`bottom`?`<div style="${i}"><div style="${r}"><div class="dot" style="${n}"></div><div class="dot" style="${n}"></div></div><div class="dot" style="${n}"></div></div>`:e===`top`?`<div style="${i}"><div class="dot" style="${n}"></div><div style="${r}"><div class="dot" style="${n}"></div><div class="dot" style="${n}"></div></div></div>`:e===`left`?`<div style="${i} flex-direction: row;"><div class="dot" style="${n}"></div><div style="${r} flex-direction: column;"><div class="dot" style="${n}"></div><div class="dot" style="${n}"></div></div></div>`:e===`right`?`<div style="${i} flex-direction: row;"><div style="${r} flex-direction: column;"><div class="dot" style="${n}"></div><div class="dot" style="${n}"></div></div><div class="dot" style="${n}"></div></div>`:``},l=(e,t,n,r)=>{let a=document.createElement(`div`);a.id=`zone-${e}`,Object.assign(a.style,o,t);let l=document.createElement(`div`);return l.id=`btn-${e}`,l.innerHTML=c(e),Object.assign(l.style,s,n),r&&(l.style.clipPath=r),l.addEventListener(`mouseenter`,()=>{l.style.backgroundColor=`rgba(0, 243, 255, 0.15)`,l.style.borderColor=`rgba(0, 243, 255, 0.8)`,l.style.boxShadow=`0 0 40px rgba(0, 243, 255, 0.25)`,l.querySelectorAll(`.dot`).forEach(e=>{e.style.background=`#fff`,e.style.boxShadow=`0 0 20px rgba(255, 255, 255, 1)`})}),l.addEventListener(`mouseleave`,()=>{l.style.backgroundColor=`rgba(8, 12, 16, 0.9)`,l.style.borderColor=`rgba(0, 243, 255, 0.4)`,l.style.boxShadow=`0 0 25px rgba(0, 243, 255, 0.1)`,l.querySelectorAll(`.dot`).forEach(e=>{e.style.background=`#00F3FF`,e.style.boxShadow=`0 0 10px rgba(0, 243, 255, 0.8)`})}),a.appendChild(l),i.appendChild(a),{zone:a,btn:l}},u=l(`left`,{top:`0`,left:`0`,width:a,height:`100%`},{top:`50%`,left:`0`,marginTop:`-${n}`,width:n,height:r,borderLeft:`none`},`polygon(0 0, 100% 25%, 100% 75%, 0 100%)`),d=l(`right`,{top:`0`,right:`0`,width:a,height:`100%`},{top:`50%`,right:`0`,marginTop:`-${n}`,width:n,height:r,borderRight:`none`},`polygon(0 25%, 100% 0, 100% 100%, 0 75%)`),f=l(`top`,{top:`0`,left:`0`,width:`100%`,height:a},{top:`0`,left:`50%`,marginLeft:`-${n}`,width:r,height:n,borderTop:`none`},`polygon(0 0, 100% 0, 75% 100%, 25% 100%)`),p=l(`bottom`,{bottom:`0`,left:`0`,width:`100%`,height:a},{bottom:`0`,left:`50%`,marginLeft:`-${n}`,width:r,height:n,borderBottom:`none`},`polygon(25% 0, 75% 0, 100% 100%, 0 100%)`);return e.appendChild(i),{left:u,right:d,top:f,bottom:p,wrapper:i}}function Ps(t,n,r,i=!0,a=!0){let o=new S(n,r.domElement);o.enableDamping=!0,o.dampingFactor=.25,o.minDistance=2,o.maxDistance=100,i&&(o.enableRotate=!1,o.enablePan=!1,o.enableZoom=!1),t.orbitControls=o,o.moveState={left:!1,right:!1,up:!1,down:!1};let s=0,c=0;if(i&&a){let e=r.domElement.parentNode,t=Ns(document.body),n=()=>{if(e){let n=e.getBoundingClientRect();t.wrapper.style.left=`${n.left}px`,t.wrapper.style.top=`${n.top}px`,t.wrapper.style.width=`${n.width}px`,t.wrapper.style.height=`${n.height}px`}};n(),window.addEventListener(`resize`,n);let i=[t.left.btn,t.right.btn,t.top.btn,t.bottom.btn],a=[t.left.zone,t.right.zone,t.top.zone,t.bottom.zone],s=()=>i.forEach(e=>{e.style.opacity=`1`,e.style.transform=`scale(1)`}),c=()=>i.forEach(e=>{e.style.opacity=`0`,e.style.transform=`scale(0.8)`});a.forEach(e=>{e.addEventListener(`mouseenter`,s),e.addEventListener(`mouseleave`,c)});let l=(e,t)=>{e.addEventListener(`mouseenter`,()=>{o.moveState[t]=!0}),e.addEventListener(`mouseleave`,()=>{o.moveState[t]=!1})};l(t.left.btn,`left`),l(t.right.btn,`right`),l(t.top.btn,`up`),l(t.bottom.btn,`down`),o.domUI=t,o.syncWrapperPosition=n,o.showEdgeUI=s,o.hideEdgeUI=c}o.edgeControlUpdate=()=>{if(!i||o.isStrategicHover){o.update();return}let t=0,r=0;if(o.moveState.left&&(t=Is),o.moveState.right&&(t=-Is),o.moveState.up&&(r=-Ls),o.moveState.down&&(r=Ls),t!==0){let e=s+t;Math.abs(e)>Rs&&(t=Math.sign(e)*Rs-s),s+=t}else Math.abs(s)>.001?(t=-(s*Fs),s+=t):s=0;if(r!==0){let e=c+r;Math.abs(e)>zs&&(r=Math.sign(e)*zs-c),c+=r}else Math.abs(c)>.001?(r=-(c*Fs),c+=r):c=0;let a=n._shakeOffset||new e.Vector3(0,0,0);if(n.position.sub(a),t!==0){let r=new e.Matrix4().makeRotationY(t);n.position.sub(o.target).applyMatrix4(r).add(o.target)}if(r!==0){let t=new e.Vector3().subVectors(n.position,o.target),i=new e.Vector3().crossVectors(n.up,t).normalize(),a=new e.Matrix4().makeRotationAxis(i,r);n.position.sub(o.target).applyMatrix4(a).add(o.target)}n.position.add(a),o.update()};let l=o.dispose;return o.dispose=()=>{o.domUI&&o.domUI.wrapper&&o.domUI.wrapper.remove(),o.syncWrapperPosition&&window.removeEventListener(`resize`,o.syncWrapperPosition),l.call(o)},o.update(),o}var Fs,Is,Ls,Rs,zs,Bs=O((()=>{Ms(),Fs=.05,Is=.01,Ls=.008,Rs=1,zs=.08}));function Vs(t){let n=new e.MeshStandardMaterial({color:16777215,metalness:.05,roughness:.2,name:`floorMat`,side:e.FrontSide,envMapIntensity:2.5});n.uniforms={...t.globalUniformsHub.uniforms,uBorderColor:{value:new e.Color(65535)}},V.environmentMap&&(n.envMap=V.environmentMap,n.envMapIntensity=2.5,V.environmentMap.mapping=e.EquirectangularReflectionMapping),n.onBeforeCompile=e=>{Sr(e,n.uniforms),Cr(e,n.uniforms),wr(e,n.uniforms)};function r(t,r,i=e.NoColorSpace){it.load(`textures/ktx2/${t}`,function(t){t.wrapS=e.RepeatWrapping,t.wrapT=e.RepeatWrapping,t.anisotropy=4,t.repeat.set(.5,4),t.colorSpace=i,n[r]=t,n.needsUpdate=!0,r===`bumpMap`&&(n.bumpScale=1.2)})}r(`hardwood2_diffuse.ktx2`,`map`,e.SRGBColorSpace),r(`hardwood2_bump.ktx2`,`bumpMap`),r(`hardwood2_roughness.ktx2`,`roughnessMap`);let i=new e.Mesh(Ws,n);return i.rotation.x=-Math.PI/2,i.receiveShadow=!0,i.position.set(3,0,-4),i.name=`floor`,i.scale.set(20,24.8,1),i.visible=!1,i}function Hs(e,t){let n=e.initialParent||e.parent,r={uuid:e.uuid,name:e.name,position:e.position.clone(),rotation:{x:e.rotation.x,y:e.rotation.y,z:e.rotation.z,order:e.rotation.order},scale:e.scale.clone(),parent:n};t.tweenData=t.tweenData||{},t.tweenData[e.uuid]=r}function Us(e){Gs.envMap=e.environment;let t=Vs(e);e.add(t),Hs(t,e);let n=e.getObjectByName(`leftWall`),r=e.getObjectByName(`rightWall`),i=e.getObjectByName(`backWall`),a=e.getObjectByName(`frontWall`),o=e.getObjectByName(`rightWall-cover`);n?n.material=Gs:console.warn(`Missing: leftWall`),r?r.material=Gs:console.warn(`Missing: rightWall`),o?o.material=Gs:console.warn(`Missing: rightWall-cover`),i?i.material=Ks:console.warn(`Missing: backWall`),a?a.material=Gs:console.warn(`Missing: frontWall`),[`rightWall`,`leftWallFoot001`].forEach(t=>{let n=e.getObjectByName(t);n&&n.material&&(n.material=n.material.clone(),n.material.uniforms=e.globalUniformsHub.uniforms,n.material.onBeforeCompile=e=>{Sr(e,n.material.uniforms)})})}var Ws,Gs,Ks,qs=O((()=>{ot(),qn(),St(),kr(),Ws=Kn.plane,Gs=new e.MeshStandardMaterial({roughness:.9,color:661043,metalness:.25,side:e.FrontSide,name:`wallMat`}),Ks=new e.MeshStandardMaterial({roughness:.9,color:`#090919`,metalness:.25,side:e.FrontSide,name:`backWallMat`}),Gs.envMapRotation.y=1.4,new e.MeshBasicMaterial({color:661043})}));function Js(t,n,r,i=null,a=null,o=`REPEAT`){let s=t?t.itemSize:r,c=new(a||(t?t.array.constructor:Float32Array))(n*s);if(i&&i.length===s)for(let e=0;e<n;e++)for(let t=0;t<s;t++)c[e*s+t]=i[t];if(t){let e=t.array,n=e.length;if(c.set(e),o===`REPEAT`)for(let t=n;t<c.length;t++)c[t]=e[t%n]}return new e.BufferAttribute(c,s)}var Ys,Xs=O((()=>{Ys=class extends e.BufferGeometry{constructor(e){super(),this.isMorphGeo=!0,this.targetGeos=e;let t=Array.isArray(e)?e:[e],n=Math.max(...t.map(e=>e.attributes.position?e.attributes.position.count:0));this.maxCount=n;let r=Math.max(...t.map(e=>e.index?e.index.count:0)),i=t.some(e=>e.attributes.uv),a=t.some(e=>e.attributes.normal);this.morphInfo=[],e.forEach(e=>{let t=null;e.index&&(t=Js(e.index,r,1,null,Uint32Array,`ZERO`));let o={position:Js(e.attributes.position,n,3),normal:a?Js(e.attributes.normal,n,3,[0,1,0]):null,uv:i?Js(e.attributes.uv,n,2):null,index:t};this.morphInfo.push(o)})}setMorphInfo(e,t=null){t??=e;let n=this.morphInfo[e],r=this.morphInfo[t];n.position&&this.setAttribute(`position`,n.position),n.normal&&this.setAttribute(`normal`,n.normal),n.uv&&this.setAttribute(`uv`,n.uv),n.index&&(this.setIndex(n.index),this.originalIndex=n.index),r.position&&this.setAttribute(`targetPosition`,r.position),r.normal&&this.setAttribute(`targetNormal`,r.normal),r.uv&&this.setAttribute(`targetUV`,r.uv),r.index&&(this.targetIndex=r.index)}addTargets(e){let t=Array.isArray(e)?e:[e];this.targetGeos.push(...t);let n=this.maxCount,r=Math.max(...t.map(e=>e.index?e.index.count:0)),i=t.some(e=>e.attributes.uv),a=t.some(e=>e.attributes.normal);t.forEach(e=>{let t=null;if(e.index){let n=Math.max(r,e.index.count);t=Js(e.index,n,1,null,Uint32Array,`ZERO`)}let o={position:Js(e.attributes.position,n,3),normal:a||this.attributes&&this.attributes.normal?Js(e.attributes.normal,n,3,[0,1,0]):null,uv:i||this.attributes&&this.attributes.uv?Js(e.attributes.uv,n,2):null,index:t};this.morphInfo.push(o)})}}})),Zs,Qs,$s=O((()=>{Zs=class e extends c{constructor(){super(e.Geometry,new l({opacity:0,transparent:!0})),this.isLensflare=!0,this.type=`Lensflare`,this.frustumCulled=!0,this.renderOrder=1/0;let r=new p,o=new p,s=new a(16,16),h=new a(16,16),g=d,_=e.Geometry,v=new u({uniforms:{scale:{value:null},screenPosition:{value:null}},vertexShader:`
    ...
    const float radius =  ${ic.toFixed(1)};
    ...
    void main() {
        ...
        if (random > 0.8) {
            ...
            vec2 pivot = vec2(${ac.y.toFixed(1)}, ${ac.z.toFixed(1)});
...
`}));async function lc(t,n){if(n&&z(window.loadingProgress||0,R(`SYS_PHYSICS_CALC`)),!t.world&&(console.warn(`Physics world not ready, waiting...`),await new Promise(e=>setTimeout(e,100)),!t.world))throw Error(`Physics World not initialized`);t.bhTargets||=[];let r=new Map;t.traverse(e=>{e.name&&r.set(e.name,e)});async function i(e,n,i={}){let a=Array.isArray(e)?e:[e],o=0,s=performance.now();for(let e of a){let c=r.get(e);if(!c)continue;i.isBhTarget&&t.bhTargets.push(c),c.visible=!1,(i.isConvexHull||performance.now()-s>1)&&(await Mr(),s=performance.now());let l=n(t,c,i);Te(t,c,l.body,l.shape,i),o++,a.length>5&&Be(`physics-binding`,.1+o/a.length*.7)}}await i([`backWall_rapier`,`rightWall`,`leftWall`,`glass2`,`frontWall`,``,`Object_15`,`Object_15001`,`Cube004`,`Cube019_3`,`Cube019_5`,`Object_1001_1`,`Object_8001`,`leftWallFoot001`,`Object_38001`],De,{bodyType:`fixed`,restitution:.4,friction:.4}),await i([`bedMain`,`bedStand`],De,{bodyType:`fixed`,restitution:.1,friction:.4,isConvexHull:!0}),Be(`physics-binding`,.2,R(`SYS_MAPPING_BOUNDARIES`));let a=r.get(`a-char`);if(a){Be(`physics-binding`,.3,R(`SYS_CHAR_COLLISION`)),new e.Quaternion().setFromAxisAngle(new e.Vector3(1,0,0),Math.PI/2);let n=_.ColliderDesc.capsule(.48,.66).setTranslation(0,.1,0);Ee(t,a,t.world.createRigidBody(_.RigidBodyDesc.kinematicPositionBased()),n,{trackBoneName:`mixamorigSpine1`,offset:new e.Vector3(0,0,0),restitution:.5,friction:.4,softKinematic:.7}),Be(`physics-binding`,.4,R(`SYS_BINDING_ARMATURES`)),[{bone:`mixamorigLeftArm`,shapeType:`capsule`,radius:.2,height:.4,offset:new e.Vector3(0,.35,0)},{bone:`mixamorigRightArm`,shapeType:`capsule`,radius:.2,height:.4,offset:new e.Vector3(0,.35,0)},{bone:`mixamorigLeftForeArm`,shapeType:`capsule`,radius:.2,height:.3,offset:new e.Vector3(0,.5,0)},{bone:`mixamorigRightForeArm`,shapeType:`capsule`,radius:.2,height:.3,offset:new e.Vector3(0,.5,0)},{bone:`mixamorigLeftUpLeg`,shapeType:`capsule`,radius:.3,height:.4,offset:new e.Vector3(0,1,0)},{bone:`mixamorigRightUpLeg`,shapeType:`capsule`,radius:.3,height:.4,offset:new e.Vector3(0,1,0)},{bone:`mixamorigRightLeg`,shapeType:`capsule`,radius:.3,height:.4,offset:new e.Vector3(0,1,0)},{bone:`mixamorigLeftFoot`,shapeType:`capsule`,radius:.24,height:.15,offset:new e.Vector3(0,.5,0)},{bone:`mixamorigRightFoot`,shapeType:`capsule`,radius:.24,height:.15,offset:new e.Vector3(0,.5,0)},{bone:`mixamorigHead`,shapeType:`ball`,radius:.43,offset:new e.Vector3(0,.35,0)}].forEach(e=>{let n;n=e.shapeType===`ball`?_.ColliderDesc.ball(e.radius):_.ColliderDesc.capsule(e.height,e.radius),e.offset&&n.setTranslation(e.offset.x,e.offset.y,e.offset.z),e.rotation&&n.setRotation(e.rotation),Ee(t,a,t.world.createRigidBody(_.RigidBodyDesc.kinematicPositionBased()),n,{trackBoneName:e.bone,restitution:.5,friction:.4,softKinematic:.7})})}else console.warn(`bindPhysics: Character 'a-char' not found.`);Be(`physics-binding`,.5,R(`SYS_ANCHORING_ROTORS`));for(let e of[`cFanBody`]){let n=r.get(e);if(n){let{body:e,shape:r}=De(t,n,{bodyType:`kinematicPosition`});Ee(t,n,e,r,{restitution:.2,friction:.9,softKinematic:.7})}}await i(`glassInvi`,De,{bodyType:`kinematicPosition`}),await i(`stool_bound`,De,{bodyType:`kinematicPosition`,isIntegrityResetTarget:!0,isIntegrityCheckTarget:!0,isConvexHull:!0,restitution:.7,friction:.2}),Be(`physics-binding`,.6,R(`SYS_DYNAMIC_RIGIDBODIES`)),await i(`Object_31`,De,{bodyType:`dynamic`,restitution:.2,mass:10,pullingDampness:.25,canSleep:!0,isBhTarget:!0,isConvexHull:!0,isIntegrityCheckTarget:!0}),await i(`pictureLionFrame`,De,{bodyType:`dynamic`,mass:1,pullingDampness:.0025,canSleep:!0,isBhTarget:!0,isConvexHull:!0,isIntegrityCheckTarget:!1,isIntegrityResetTarget:!0}),await i(`Model_0001`,De,{bodyType:`dynamic`,mass:1.5,restitution:.01,friction:.995,pullingDampness:.0025,canSleep:!0,isBhTarget:!0,isConvexHull:!0,isIntegrityResetTarget:!0,isIntegrityCheckTarget:!1}),await i(`blackCat`,Oe,{bodyType:`fixed`,scale:new e.Vector3(1,1,.5),offset:new e.Vector3(0,.5,0)}),Be(`physics-binding`,.7,R(`SYS_BONE_HIERARCHIES`));let o=r.get(`GLTF_created_0001`);if(o){let n=_.ColliderDesc.capsule(.2,.15).setTranslation(0,0,0);Ee(t,o,t.world.createRigidBody(_.RigidBodyDesc.kinematicPositionBased()),n,{trackBoneName:`Root_M_2_6_11`,offset:new e.Vector3(0,0,0),restitution:.2,friction:.9,softKinematic:.7}),[{bone:`HipFix_R_3_7_12`,shapeType:`ball`,radius:.1,offset:new e.Vector3(0,0,0)},{bone:`HipFix_L_85_89_94`,shapeType:`ball`,radius:.1,offset:new e.Vector3(0,0,0)},{bone:`RootPart1_M_16_20_25`,shapeType:`capsule`,radius:.4,height:.3,offset:new e.Vector3(0,.2,.2)},{bone:`Tail0_M_10_14_19`,shapeType:`capsule`,radius:.05,height:.2,offset:new e.Vector3(0,0,0)},{bone:`Tail20_M_15_19_24`,shapeType:`capsule`,radius:.05,height:.2,offset:new e.Vector3(0,0,0)},{bone:`Head_M_26_30_35`,shapeType:`ball`,radius:.3,offset:new e.Vector3(0,0,0)}].forEach(e=>{let n;n=e.shapeType===`ball`?_.ColliderDesc.ball(e.radius):_.ColliderDesc.capsule(e.height,e.radius),e.offset&&n.setTranslation(e.offset.x,e.offset.y,e.offset.z),Ee(t,o,t.world.createRigidBody(_.RigidBodyDesc.kinematicPositionBased()),n,{trackBoneName:e.bone,restitution:.2,friction:.9,softKinematic:.7})})}await i(`Object_2001`,De,{bodyType:`dynamic`,mass:80,restitution:.6,canSleep:!0,isBhTarget:!0,isConvexHull:!0,offset:new e.Vector3(0,0,0),pullingDampness:.45,isIntegrityCheckTarget:!0}),await i(`mjolnir_low_mjolnir_hammer_0`,De,{bodyType:`dynamic`,mass:10,restitution:0,canSleep:!0,isBhTarget:!0,pullingDampness:.9075,isConvexHull:!0,isIntegrityResetTarget:!0}),await i(`questionCube`,Oe,{bodyType:`dynamic`,mass:20,isBhTarget:!0,isIntegrityResetTarget:!0}),await i(`shelf`,De,{bodyType:`dynamic`,mass:400,restitution:.3,canSleep:!0,isBhTarget:!0,pullingDampness:.25,isIntegrityCheckTarget:!1,isIntegrityResetTarget:!0});let s=[];for(let e=0;e<=38;e++){let t=`book`+String(e).padStart(3,`0`);s.push(t)}await i(s,Oe,{bodyType:`dynamic`,isBhTarget:!0,mass:2,restitution:.05,canSleep:!0,pullingDampness:.25,isIntegrityCheckTarget:!1,isIntegrityResetTarget:!0}),await i([`pokeball`,`pokeball2`],ke,{bodyType:`dynamic`,mass:27,scale:.425,restitution:.9,isBhTarget:!0,isIntegrityResetTarget:!0}),await i(`drone`,Oe,{bodyType:`kinematicPosition`,mass:2.5,isBhTarget:!1,linearDamping:1,angularDamping:1}),Be(`physics-binding`,.85,R(`SYS_COLLISION_MESHES`));try{await i(`caseCover`,Oe,{bodyType:`dynamic`,mass:100,restitution:.1,isBhTarget:!0,isIntegrityCheckTarget:!0}),await i(`Object_42001`,Oe,{bodyType:`dynamic`,mass:.5,restitution:.93,canSleep:!0,isBhTarget:!0,pullingDampness:-1,isIntegrityResetTarget:!0}),await i(`screenDisplay`,Oe,{bodyType:`dynamic`,mass:200,scale:new e.Vector3(1,1.05,.9),offset:new e.Vector3(0,-.13,0),isBhTarget:!0,pullingDampness:.5,isIntegrityCheckTarget:!0}),await i(`screenDisplay2`,Oe,{bodyType:`dynamic`,mass:20,scale:new e.Vector3(1,1.05,.9),offset:new e.Vector3(0,-.13,0),isBhTarget:!0,pullingDampness:.15,restitution:.3,isIntegrityCheckTarget:!0}),await i(`verticalMonitor`,Oe,{bodyType:`dynamic`,mass:150,friction:.9,scale:new e.Vector3(1,1,1),offset:new e.Vector3(0,1.75,0),isBhTarget:!0,pullingDampness:.25,isIntegrityCheckTarget:!0}),await i([`aegis`,`aegis2`],De,{bodyType:`dynamic`,mass:1.1,restitution:.01,canSleep:!0,isBhTarget:!0,isConvexHull:!0,isIntegrityResetTarget:!0}),await i([`pillow-small-1`,`pillow-small-2`,`pillow-big-1`,`pillow-big-2`],De,{bodyType:`dynamic`,mass:100.3,restitution:0,friction:.9,canSleep:!0,pullingDampness:.64,isBhTarget:!0,isConvexHull:!0,isIntegrityResetTarget:!0})}catch(e){throw e}}var uc=O((()=>{Fe(),ot(),Nr(),Ce()})),dc=O((()=>{hn(),At(),qn()}));function fc(t,n){if(!t||!n)return console.error(`initConversationBox: Missing model or scene`),null;let r=null;if(t.traverse(e=>{e.isBone&&e.name===`mixamorigSpine2`&&(r=e)}),r||t.traverse(e=>{e.isBone&&e.name===`mixamorigSpine`&&(r=e)}),r||t.traverse(e=>{e.isBone&&e.name===`mixamorigHips`&&(r=e)}),r||=t.getObjectByName(`Ch23_Hair`),!r)return console.warn(`initConversationBox: No head found.`),null;let i=null,a=0,o=0,s=null;new e.Vector2;function c(){if(document.getElementById(`scifi-shout-box`))return;i=document.createElement(`div`),i.id=`scifi-shout-box`,Object.assign(i.style,{position:`absolute`,top:`0`,left:`0`,pointerEvents:`none`,zIndex:`5000`,padding:`10px 20px`,color:`#ffffff`,fontFamily:`"Rajdhani", sans-serif`,fontWeight:`600`,fontSize:`18px`,textAlign:`center`,display:`flex`,alignItems:`center`,justifyContent:`center`,lineHeight:`1.4`,backgroundColor:`rgba(5, 10, 25, 0.85)`,backdropFilter:`blur(10px)`,border:`1.5px solid #00F0FF`,boxShadow:`0 0 30px rgba(0, 240, 255, 0.25)`,opacity:`0`,whiteSpace:`pre-wrap`,transform:`translate(-50%, -100%) scale(0.8)`,transition:`opacity 0.3s ease, transform 0.3s cubic-bezier(0.175, 0.885, 0.32, 1.25)`,letterSpacing:`1px`,textTransform:`none`,borderRadius:`2px`,clipPath:`none`});let e=document.createElement(`div`);Object.assign(e.style,{position:`absolute`,bottom:`-7.5px`,left:`50%`,width:`12px`,height:`12px`,backgroundColor:`rgba(5, 10, 25, 0.95)`,borderLeft:`1.5px solid #00F0FF`,borderBottom:`1.5px solid #00F0FF`,transform:`translateX(-50%) rotate(-45deg)`,zIndex:`1`}),i.appendChild(e);let t=document.createElement(`div`);Object.assign(t.style,{position:`absolute`,bottom:`4px`,right:`4px`,width:`8px`,height:`8px`,borderRight:`2px solid #00F0FF`,borderBottom:`2px solid #00F0FF`}),i.appendChild(t),document.getElementById(`experience-container`).appendChild(i)}function l(e,t={}){i||c();let n=e;Array.isArray(e)&&(n=e[Math.floor(Math.random()*e.length)]);let r=i.querySelector(`.text-content`);r||(r=document.createElement(`span`),r.className=`text-content`,i.insertBefore(r,i.firstChild)),r.innerText=n;let a=1;t.extraSmall?a=.56:t.small&&(a=.65),i.style.fontSize=`${18*a}px`,i.style.minWidth=n.length>20?`160px`:`95px`}c();let u={update:t=>{if(i){if(Math.abs(o-a)>.01){let e=3*t;o=o<a?Math.min(o+e,a):Math.max(o-e,a),i.style.opacity=o,i.style.display=o>.01?`block`:`none`}else i.style.opacity=a,i.style.display=a>.01?`block`:`none`;if(a>.01&&r&&n.camera&&n.renderer){let t=new e.Vector3;r.getWorldPosition(t),t.y+=2,t.project(n.camera);let a=n.renderer.domElement,s=a.clientWidth/2,c=a.clientHeight/2,l=t.x*s+s,u=-(t.y*c)+c,d=.8+.2*o;i.style.transform=`translate(-50%, -100%) translate(${l}px, ${u}px) scale(${d})`}}},updateText:(e,t={})=>{l(e,t)},show:()=>{a=1,i&&(i.style.display=`block`)},hide:()=>{a=0},shout:(e,t=3e3,n={})=>(s&&clearTimeout(s),l(e,n),u.show(),s=setTimeout(()=>{u.hide(),s=null},t),s),clear:()=>{s&&=(clearTimeout(s),null),a=0,o=0,i&&(i.style.opacity=`0`,i.style.display=`none`)}};return u}var pc=O((()=>{}));function mc(t){let n=t.getObjectByName(`Blackhole`);if(!n)return;n.position.y=-500,n.scale.setScalar(2),n.traverse(t=>{t.isMesh&&t.material&&(t.material.roughness=.95,t.material.metalness=0,t.material.side=e.FrontSide,t.castShadow=!1)});function r(e,t,r){let i=n.getObjectByName(e);i?.material&&i.material[t]!==void 0&&(i.material[t]=r)}r(`Lathe_L_Blackhole_03_0`,`roughness`,.4),r(`Lathe_S_Blackhole_01_0`,`metalness`,.6);let i=n.getObjectByName(`Lathe_Center`);if(i){let n=t.globalUniformsHub;n&&n.core,i.material=new e.ShaderMaterial({vertexShader:Gt,fragmentShader:Qt,transparent:!0,uniforms:{...n.uniforms,nebulaCoreRadius:{value:20},nebulaTwistFactor:{value:0},alpha:{value:1}},blending:e.AdditiveBlending,name:`nebulaMat`})}}function hc(t){let n=t.globalUniformsHub,r=n?n.uniforms:{},i=Pr(t,$t,{side:e.BackSide,uniforms:{...n.core,isStriking:r.isStriking||{value:!1},normalizedStrikePos:r.normalizedStrikePos||{value:new e.Vector2(-2,-2)},uRainHeaviness:r.uRainHeaviness||{value:2},uStormSharpness:r.uStormSharpness||{value:0},uMoonPosition:r.uMoonPosition||{value:new e.Vector2(.58,.705)},uMoonSize:r.uMoonSize||{value:.006},uMoonBrightness:r.uMoonBrightness||{value:2.5},uMoonBlur:r.uMoonBlur||{value:0},uCraterScale:r.uCraterScale||{value:.555},uCraterIntensity:r.uCraterIntensity||{value:.28},uFarMountainOffset:r.uFarMountainOffset||{value:0},uNearMountainOffset:r.uNearMountainOffset||{value:-.5}}}),a=new e.Mesh(Kn.plane,i);t.add(a),a.position.set(-55,-20,30),a.scale.setScalar(150),a.name=`planeSky`,a.visible=!1}function gc(t){let n=t.globalUniformsHub,r=n?n.uniforms:{},i=Pr(t,en,{transparent:!0,uniforms:{rainGlassOpacity:r.rainGlassOpacity||{value:1},glassRainAmount:r.glassRainAmount||{value:1},uRimCenter:r.uRimCenter||{value:new e.Vector2(-.5,.5)},uRainOffset:r.uRainOffset||{value:0},uWaterIntensity:n&&n.uWaterIntensity||{value:.2},hasRimOnGlass:{value:!0}},blending:e.AdditiveBlending,side:e.FrontSide}),a={vertexShader:i.vertexShader,fragmentShader:i.fragmentShader,uniforms:i.uniforms};Sr(a,i.uniforms),i.vertexShader=a.vertexShader,i.fragmentShader=a.fragmentShader;let o=t.getObjectByName(`glass1`),s=t.getObjectByName(`glass2`);o&&(o.material=i),s&&(s.material=i)}var _c,vc=O((()=>{ot(),st(),qn(),At(),qs(),Nn(),nc(),hn(),cc(),Fe(),Ja(),Zr(),uc(),ec(),dc(),kr(),Zo(),pc(),Nr(),Ce(),jt(`#dcd0ba`,.85,.03,6.5),Mt(`#dcd0ba`,1,1),_c=class{constructor(e,t,n,r){this.scene=e,this.camera=t,this.renderer=n,this.model=null,this.mixer=null,this.constantUniform=kt,this.resources=r}async init(t,n,r={}){return new Promise(async(n,r)=>{let i=this.resources.roomModel;if(!i){let e=`Resources: Room Model not loaded.`;console.error(e),t&&z(window.loadingProgress||0,R(`SYS_ERROR`)),r(e);return}try{t&&z(window.loadingProgress||0,R(`SYS_INIT_SCENE`)),Be(`model-assembly`,.1),this.model=i.scene,this.model.name=`roomGLBModel`,this.model.visible=!1,this.scene.add(this.model),this.scene.room=this.model,this.scene.animations=i.animations,this.boxUpdater=fc(this.model,this.scene),this.scene.conversationManager=this.boxUpdater;let a=[`leftWallFoot001`,`Cube004`,`shelf`,`glass1`,`Object_17`,`Object_1001_1`,`Object_8001`,`glass1`,`pillow-big-2`,`pillow-small-2`,`pillow-small-1`,`pillow-big-1`,`bedMain`,`Ch23_Hair`,`Ch23_Suit`,`Object_15`],o=[`Object_1001_1`,`Object_8001`,`Object_17`,`leftWallFoot001`];this.model.getObjectByName(`inviMesh`).material.visible=!1;let s=[this.model],c=0,l=performance.now();for(;s.length>0;){let t=s.pop();if(t.isMesh&&(t.material.side=e.FrontSide,a.includes(t.name)&&(t.receiveShadow=!0),o.includes(t.name)||t.name.startsWith(`book`)?t.castShadow=!1:t.castShadow=!0),t.children)for(let e=t.children.length-1;e>=0;e--)s.push(t.children[e]);c++,c%50==0&&performance.now()-l>4&&(await Mr(),l=performance.now())}if(this.mixer=i.mixer,this.scene.mixer=i.mixer,this.scene.heroClips=i.heroClips,this.scene.activeAction=i.activeAction,this.mixer&&this.scene.animations){let t=e.AnimationClip.findByName(this.scene.animations,`3|PlaneAction`);t&&(this.scene.fanAction=this.mixer.clipAction(t),this.scene.fanAction.play())}B.start(`Helpers Setup`),Us(this.scene),mc(this.scene),hc(this.scene),gc(this.scene),On(this.scene),tc(this.scene);let u=rc(this.scene);this.scene.getObjectByName(`blackholeScene`).attach(u),B.end(`Helpers Setup`),B.start(`Adjust Objects`),Fr(this.scene,t).then(()=>(B.end(`Adjust Objects`),B.start(`Bind Physics`),lc(this.scene,t))).then(async()=>(B.end(`Bind Physics`),Be(`physics-binding`,.9),await Ir(this.scene),Be(`physics-binding`,1),wa(this.scene),this.scene.constantUniform=kt,t&&z(window.loadingProgress||0,R(`SYS_FINALIZE`)),Promise.resolve())).then(()=>{z(100),clearInterval(void 0),this.model&&this.model.position.set(0,-5e4,0),n()}).catch(e=>{console.error(`Critical error in async chain:`,e),r(e)})}catch(e){console.error(`Critical error during scene initialisation:`,e),r(e)}finally{window.completeTask&&(completeTask(`model-assembly`),completeTask(`physics-binding`))}})}updateAnimationMixer(e){this.mixer&&this.mixer.update(e),this.boxUpdater&&this.boxUpdater.update(e)}}}));async function yc({scene:e,camera:t,orbitControl:n,clock:r,pointsApp:i}){e.targetAnimHz=30,Fo(e,0),Eo(e,bc*.05),Z(R(`SYS_INIT`)),Z(R(`ENV_CALIBRATION`)),await bo(),Z(R(`SYS_PILOT_ENTRY_WAIT`)),await xo(),await X(200),e.isTransitioning=!0,e.HUD&&typeof e.HUD.breathe==`function`&&e.HUD.breathe(I.ELECTRIC_CYAN);let a=Q.currentMode;e.fpsStats&&e.fpsStats.avg<45&&(e.isLowPowerMode=!0),Z(`Protocol ${a.toUpperCase()} verified.`);let o=document.getElementById(`cv-container`);o&&o.classList.contains(`collapsed`)&&(o.classList.remove(`collapsed`),window.dispatchEvent(new CustomEvent(`cvToggle`,{detail:{collapsed:!1}})),await X(500)),e.HUD&&typeof e.HUD.runTweenOpen==`function`&&(await e.HUD.runTweenOpen(1500,{isIncludedIsland:!1,isIncludedDecos:!1}),e.HUD.runTweenShowIsland(2500),eo.onHudOpen(e),e.HUD.runTweenShowDecos(1e3)),i&&(i.playIntro(),i.activateScrollInteractions(),i.triggerStep&&i.triggerStep(0,1500,!0)),Z(R(`SYS_BUILD_START`)),setTimeout(()=>{e.isTransitioning=!1},1e3)}var bc,xc=O((()=>{Zo(),ys(),_s(),Ce(),lr(),Ja(),be(),to(),bc=Wo}));function Sc(t,n){if(!t)return console.warn(`[knowhere] Parent not found. Skipping.`),null;let r=new e.PlaneGeometry(1,1),i=Pr(n,wc,{transparent:!0,blending:e.NormalBlending,side:e.DoubleSide,depthTest:!0,depthWrite:!1,vs:Cc});i.uniforms&&(i.uniforms.uScaleFactor={value:0},i.uniforms.uHudOffset={value:new e.Vector2(0,1.2)},i.uniforms.uStarScreenPos={value:new e.Vector2(0,0)});let a=new e.Mesh(r,i);return a.name=`knowhere`,a.scale.set(10,10,1),a.frustumCulled=!1,t.add(a),n.knowhere=a,a}var Cc,wc,Tc=O((()=>{Zr(),At(),Cc=`
    varying vec2 vUv;
