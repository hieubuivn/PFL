...
`||c*(r+5)>t?i+=n:i+=`ABCDEFGHIJKLMNOPQRSTUVWXYZ0123456789$#@%&*`[Math.floor(Math.random()*42)]}t.textContent=i}c<1?Os=requestAnimationFrame(o):t.textContent=e};Os=requestAnimationFrame(o)}}function bs(e=!1){if(Os&&cancelAnimationFrame(Os),Cs||=document.getElementById(`stat-subtitle`),Cs){if(Cs.classList.remove(`active`),e){Cs.innerHTML=``;return}setTimeout(()=>{Cs.classList.contains(`active`)||(Cs.innerHTML=``)},650)}}var xs,Ss,Cs,ws,Ts,Es,Ds,Os,ks=M((()=>{gs(),ws=0,Ts=0,Es=0})),As,js=M((()=>{pt(),As=class{constructor(e){this.canvas=document.getElementById(e),this.gl=this.canvas.getContext(`webgl2`,{alpha:!0,premultipliedAlpha:!0,antialias:!0}),this.gl||=(console.error(`WebGL2 not supported, falling back to WebGL1 (expect artifacts)`),this.canvas.getContext(`webgl`)),this.uProgress=0,this.targetProgress=0,this.mouse={x:.5,y:.5},this.textures={poba:null,dev:null},this.init()}async init(){console.log(`[AvatarEngine] Initializing WebGL2 Lab (KTX2 Edition)...`),await this.loadTextures(),this.setupProgram(),this.onResize(),window.addEventListener(`resize`,()=>this.onResize()),window.addEventListener(`mousemove`,e=>this.onMouseMove(e)),requestAnimationFrame(e=>this.render(e)),console.log(`[AvatarEngine] Engine Ready.`)}async loadTextures(){let e=e=>new Promise((t,n)=>{dt.transcoderPath||dt.setTranscoderPath(`https://cdn.jsdelivr.net/npm/three@0.170.0/examples/jsm/libs/basis/`),window._ktx2SupportDetected||(dt.detectSupport({capabilities:{isWebGL2:!0},extensions:{has:e=>{try{return!!this.gl.getExtension(e)}catch{return!1}},get:e=>{try{return this.gl.getExtension(e)}catch{return null}}}}),window._ktx2SupportDetected=!0),dt.load(e,e=>{let n=this.gl,r=n.createTexture();n.bindTexture(n.TEXTURE_2D,r),e.mipmaps.forEach((t,r)=>{n.compressedTexImage2D(n.TEXTURE_2D,r,e.format,t.width,t.height,0,t.data)}),n.texParameteri(n.TEXTURE_2D,n.TEXTURE_WRAP_S,n.CLAMP_TO_EDGE),n.texParameteri(n.TEXTURE_2D,n.TEXTURE_WRAP_T,n.CLAMP_TO_EDGE),n.texParameteri(n.TEXTURE_2D,n.TEXTURE_MIN_FILTER,e.mipmaps.length>1?n.LINEAR_MIPMAP_LINEAR:n.LINEAR),n.texParameteri(n.TEXTURE_2D,n.TEXTURE_MAG_FILTER,n.LINEAR),t(r)},void 0,t=>{console.error(`Failed to load KTX2:`,e,t),n(t)})}),t=`${BASE}textures/ktx2/avatars-celShaded.ktx2`;this.textures.atlas=await e(t)}setupProgram(){this.program=this.createProgram(`#version 300 es
        ...
        `,`#version 300 es
            ...
            vec4 sampleAtlas(vec2 uv, bool isPoba) {
                // UVs from vsSource are already flipped (vUv.y = 1.0 - vUv.y)
                // Atlas is 2x1: [POBA | DEV]
                // Each persona has 0.5 width
                vec2 atlasUv = vec2(uv.x * 0.5 + (isPoba ? 0.0 : 0.5), uv.y);
                
                // Safety clamp to avoid edge bleeding between personas
                float minX = isPoba ? 0.0 : 0.5;
                float maxX = isPoba ? 0.5 : 1.0;
                atlasUv.x = clamp(atlasUv.x, minX + 0.001, maxX - 0.001);
                
                return texture(uTexAtlas, atlasUv);
            }
            ...
            void main() {
                ...
                if (sweepVal < 0.999) texDev = sampleAtlas(vUv + vec2(-drift, 0), false);
                if (sweepVal > 0.001) {
                    texPoba = sampleAtlas(vUv + vec2(drift, 0), true);
                    // Aberration only in glitch zone
                    if (glitchZone > 0.01) texPoba.r = sampleAtlas(vUv + vec2(drift + 0.03 * glitchZone, 0.0), true).r;
                }
                ...
                float o = 0.02; 
                float aDev = min(min(sampleAtlas(vUv + vec2(o, 0), false).a, sampleAtlas(vUv - vec2(o, 0), false).a),
                                 min(sampleAtlas(vUv + vec2(0, o), false).a, sampleAtlas(vUv - vec2(0, o), false).a));
                float aPoba = min(min(sampleAtlas(vUv + vec2(o, 0), true).a, sampleAtlas(vUv - vec2(o, 0), true).a),
                                  min(sampleAtlas(vUv + vec2(0, o), true).a, sampleAtlas(vUv - vec2(0, o), true).a));
