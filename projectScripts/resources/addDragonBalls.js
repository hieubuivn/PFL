...
import { registerInMap } from '../scenario/scenarioUtility.js';
import { spawnBitcoin } from './spawnBitcoin.js';
import { updateStory } from '../utils/status.js';
import { shootDroneBeam, getObj } from '../scenario/scenarioUtility.js';
...
export function addDragonBall(scene, numberOfStars = 4) {
    ...
    registerInMap(scene, ballMesh);
    ballMesh.castShadow = true
    ballMesh.ignoreRaycast = true
    ballMesh.userData.isPhysicsBall = true // Performance flag for raycaster filtering
    ballMesh.userData.isDragonBall = true;
    ballMesh.userData.starCount = numberOfStars;
    ...
    // add physics rapier
    // GUARD: If Rapier is already doing something (stepping/rendering), skip this frame
    // to avoid the "recursive use" aliasing error in WASM.
    if (scene.world.isBusy) {
        // Defer slightly and try again
        setTimeout(() => addDragonBall(scene, numberOfStars), 16);
        scene.remove(ballMesh); // Cleanup our mesh as we will recreate it
        return null;
    }

    scene.world.isBusy = true;
    let body;
    //set body mass to 0.01
    try {
    ...
    RAYCAST.addRaycastObject(
        ...
        ballMesh,
        {
            onMouseEnter: (ballMesh) => {
                ...
                const key = `UI_INFORMER_DRAGONBALL_${numberOfStars}`;
                if (scene.raycasterWrapper?.mouseInContainer) {
                    RAYCAST.setInformerBg(scene, B64.punch, getDynamicText(key));
                }
...
function moveToLocation(dynamicBody, targetPosition, forceScalingFactor = 10) {
    ...
    dynamicBody.applyImpulse({ x: impulse.x, y: impulse.y, z: impulse.z }, true);
}
...
export function toggleDragonBallGravity(scene, clickedObject, intersection) {
    ...
    // 3. Trigger Drone Ritual Beam (Orange)
    const dbOrange = 0xffcc00; // Brilliant Gold/Orange
    shootDroneBeam(scene, clickedObject, "", null, `db-click-${clickedObject.uuid}`, false, dbOrange, false, 500);

    // 4. Explode the balls if gravity was turned OFF
    if (!isGravityActive) {
        explodeBalls(scene);
    }

    // 4. ANOMALOUS RITUAL: Multi-Coin Spawn + Radial Pulse + Instant Eye
    const ritualPos = clickedObject.position.clone();
    
    // a. Spawn 1-5 coins instead of just 1
    const coinCount = Math.floor(Math.random() * 5) + 1;
    for (let i = 0; i < coinCount; i++) {
        const coinFlip = {
            x: (Math.random() - 0.5) * 12.0,
            y: 18.0 + Math.random() * 10.0,
            z: (Math.random() - 0.5) * 12.0
        };
        spawnBitcoin(scene, ritualPos.clone().add({x:0, y:0.5, z:0}), coinFlip);
    }

    // b. Radial Gravitational Ripple (Affect nearby bodies)
    if (scene.physicBodies) {
        scene.physicBodies.forEach(body => {
            if (!body.threeObject || body.threeObject === clickedObject) return;
            const dist = body.threeObject.position.distanceTo(ritualPos);
            if (dist < 10.0) {
                const dir = new THREE.Vector3().subVectors(body.threeObject.position, ritualPos).normalize();
                const pushMag = (1.0 - dist / 10.0) * 8.0; 
                body.applyImpulse({ x: dir.x * pushMag, y: pushMag * 1.5, z: dir.z * pushMag }, true);
            }
        });
    }

    // c. High-Speed Eye Activation (1.0s instead of 3s)
    if (typeof openDragonEye === 'function') {
        openDragonEye(scene, 1000); 
        
        if (scene._dragonEyeTimeout) clearTimeout(scene._dragonEyeTimeout);
        scene._dragonEyeTimeout = setTimeout(() => {
            if (typeof closeDragonEye === 'function') closeDragonEye(scene, 2000);
        }, 5000);
    }

    // d. Narrative Signal
    if (typeof updateStory === 'function') {
        updateStory(`ANOMALY_0${clickedObject.userData.starCount || 7}: DIMENSIONAL_KEY_ACTIVE`);
    }
}
...
function explodeBalls(scene) {
    ...
    scene.world.ballBodies.forEach((body) => {
        ...
        body.applyImpulse({ x: force.x, y: force.y, z: force.z }, false);
    });
}
